...
def __replace_with_dict(str_to_replace: str, replacement_dict: dict) -> str:
    str_replaced = reduce(
        ...
        [str_to_replace, *list(replacement_dict.items())]
    )
    return str_replaced
...
def clean_tsv_file(in_path: str, out_path: str) -> None:
    ...
    with open(in_path, encoding="utf-8", mode="r") as f1, open(
        out_path, encoding="utf-8", mode="w "
    ) as f2:
        for line in f1:
            try:
            ...
            if name:
                line_cleaned = f"{name}\t{text_cleaned}\n"
            else:
                line_cleaned = f"{text_cleaned}\n"
    ...
    with open(in_path, encoding="utf-8", mode="r") as f1, open(
        ...
        for line in f1:
            try:
                ...
                word_replaced = __replace_with_dict(
                    word,
                    replacement_dict
                    ).strip()
                if word_replaced != "":
...
def clean_json_file(in_path: str, out_path: str) -> None:
    ...
    with open(in_path, encoding="utf-8", mode="r") as f1, open(
        ...
        for row in data["words"]:
            new_row = {
                "word": __replace_with_dict(
                    row["word"],
                    replacement_dict
                    ).strip(),
                "punctuation": __replace_with_dict(
                    row["punctuation"], replacement_dict
                ).strip(),
                "space_after": row["space_after"],
            }
            clean_rows.append(new_row)

        data_clean = {"title": data["title"], "words": clean_rows}
